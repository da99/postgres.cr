...
describe Postgres::Connection, "#escape_literal" do
  ...
  #  expect_raises(Postgres::Connection_Error) { escape_literal("\u{F4}") }
  # end
end

describe Postgres::Connection, "#escape_identifier" do
  it { escape_identifier(%(foo)).should eq(%("foo")) }
  it { escape_identifier(%(what's \\ your "name")).should eq(%("what's \\ your ""name""")) }
  # it "raises on invalid strings" do
  #  expect_raises(Postgres::Connection_Error) { escape_identifier("\u{F4}") }
  # end
end
